...
// .andExpect(jsonPath("$.content.length()").value(2)) // Verify the number of
// elements in the returned
// // page
// .andExpect(jsonPath("$.content[0].id").value("id1")) // Verify the id of the
// first element
// .andExpect(jsonPath("$.content[0].name").value("Sparrow")) // Verify the name
// of the first element
// .andExpect(jsonPath("$.content[0].color").value("Brown")); // Verify the
// color of the first element
...
// // mockMvc.perform(MockMvcRequestBuilders.get("/birds/{id}", 1) // Perform a
// GET
// // request to /birds/{id}
// // .contentType(MediaType.APPLICATION_JSON)) // Set content type to JSON
// // .andExpect(MockMvcResultMatchers.status().isOk()) // Verify that response
// // status is OK (200)
// // .andExpect(MockMvcResultMatchers.jsonPath("$.id").value(1)); // Verify
// JSON
